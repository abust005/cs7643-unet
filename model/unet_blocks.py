  ...
  def __init__(self, in_channels, out_channels, padding='same', padding_mode='reflect'):
    ...
    self.module_list = nn.Sequential(
      nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, 
                padding=padding, padding_mode=padding_mode, stride=1),
      nn.ReLU(),
...
class UpConv(nn.Module):
  def __init__(self, in_channels):
    ...
    self.module_list = nn.ModuleDict(
      {
        'up_conv' : nn.ConvTranspose2d(in_channels=in_channels, out_channels=in_channels//2, kernel_size=2, stride=2),
