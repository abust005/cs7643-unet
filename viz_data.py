...
if __name__ == '__main__':
  ...
  for b in range(0, 10):
      ...
      for i in range(0, batch_size):
        ...
        #   fig.delaxes(axs[2,1])
          axs[0, 0].imshow(torch.moveaxis(singleY[i], 0, -1), cmap=plt.get_cmap('tab10'), norm=b_norm)
          axs[0, 1].imshow(torch.moveaxis(center_crop(singleY[i], [52, 52]), 0, -1), cmap=plt.get_cmap('tab10'), norm=b_norm)
